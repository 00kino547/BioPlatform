...
export function PlatformIcon({ platform, className = "h-4 w-4", color = "currentColor" }: PlatformIconProps) {
  const key = platform.toLowerCase();
  const renderer = icons[key];
  if (!renderer) {
    return (
      <span className={`${className} flex items-center justify-center text-[10px] font-bold`} style={{ color }}>
        {platform.charAt(0).toUpperCase()}
      </span>
    );
  }
  return <span className={className}>{renderer(color)}</span>;
}
