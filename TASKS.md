...
- [x] GitHub issue #16 — user role missing `profiles.customDomain` permission → seeded/updated role permissions grant it (migration `docs/migrations/2026-08-27_add-customdomain-to-user-role.sql`)
- [x] GitHub issue #17 — ACME improvements: admin **Remove** action (`DELETE /api/admin/custom-domains/:id`, deletes record + cleans up cert files/nginx config) + 5-minute issuance timeout (stuck "pending" now fails with a stored `tlsError`; admin TLS column shows "failed — hover for details")
- [x] Custom domains for FREE tier — optional `CUSTOM_DOMAINS_ALL_TIERS` env var (default `false`) lifts the PRO/ENTERPRISE tier gate for all tiers; PRO/ENTERPRISE behavior unchanged
- [x] Sitemap improvements — `/sitemap.xml` now includes static pages (`/privacy`, `/terms`, `/api-docs`) with priorities/changefreq, profile URLs at priority 0.7; serves a sitemap index with per-page sub-sitemaps (`/sitemap-<n>.xml`, nginx-proxied) above 50k public profiles
- [x] Discoverable (resident) passkey login — "Login with passkey" button on the login identifier stage (no username needed) via new `/auth/login/passkey/discoverable/*` endpoints that resolve the account from the credential ID
- [x] Passkey authenticator info — records browser-reported `authenticatorAttachment` + SimpleWebAuthn `credentialDeviceType` per passkey (migration `docs/migrations/2026-09-01_passkey-authenticator-info.sql`), displayed in the Security tab as reported (This device / Security key, Device-bound / Synced across devices); NOT claimed to prove discoverable status — `residentKey` remains the requested type
- [x] Passkey residency verification — records `resident_verified_at` on each passkey at every successful discoverable (allowedCredentials-empty) login and never derives discoverability from registration (migration `docs/migrations/2026-09-01_passkey-residency-verification.sql`); admin users list exposes per-user `passkeyCount`/`passkeysFreshCount`/`residentPasskeyCount`/`residentFreshCount`, `hasNoPasskeys`, `passkeysUnverified`, `securityFlag` (`none`/`no-passkeys`/`passkeys-unverified`) with a Users-tab security section + row badges; TTL configurable via `PASSKEY_RESIDENCY_TTL_DAYS` (default 14, sliding) and no-passkey flag toggleable via `ADMIN_FLAG_USERS_WITHOUT_PASSKEYS` (default true); Privacy Policy discloses the stored verification timestamp
- [x] Update-checker pre-release toggle — `UPDATE_CHECK_INCLUDE_PRERELEASES` (default `false`): pre-releases excluded from updates/severity/lockdown everywhere and surfaced only as a minimal admin-panel notification (`prereleaseAvailable`/`prereleaseCount`/`prereleaseLatest` in `/api/version`); when `true`, pre-releases count as normal updates, can raise severity to `security` (lockdown) but never `critical`; `.env.example` + docker-compose + en/es environment-variables docs parity
- [x] Terms of Service update — Discord presence/Post-to-Discord in section 7, new sections 8 (Email Notifications) and 9 (Access to the API), renumbered 10–14
- [x] Privacy Policy update — audio-upload disclosure in section 4, new section 5g (Email Notifications)
- [x] Windows Docker build/CLI parity — `pnpm docker:build` and `pnpm cli` dispatch via Node launchers to the platform script (`build.sh`/`build.ps1`, `bioplatform.sh`/`bioplatform.ps1`), so Windows users can build images and run the admin CLI without creating their own scripts; `bioplatform.ps1` hardened with strict exit-code handling; Dockerfiles run `apk upgrade` on the runtime base to keep OS packages patched (backend `node:22-alpine` + frontend `nginx:alpine`)
...
- [x] Security audit round 5 (full-stack, 16-subagent) — H1: JWT purpose bypass fix (require `purpose === "auth"`); H2: 2FA/passkey enumeration fix (uniform `/login/start` response + frontend passkey button always enabled); M2: WebAuthn challenge TTL enforcement + atomic consumption; M4: Discord webhook error sanitization; M5: Invite validation + uniqueness check moved inside transaction; M6: Atomic `increment` in auth failure recording; M7: MusicTrack `(profileId, position)` unique constraint; M8: 5-min email notification cooldown per profile; M9: Docker HEALTHCHECK; M10: pnpm pinned to 11.9.0; M11: Registration generic error (no field enumeration); M12: Profile limit check in transaction; L5: Domain status enum removed from admin errors; L6: Analytics GROUP BY LIMIT; L8: Frontend depends_on service_healthy; L9: sed_escape newline handling; L12: ADMIN_EMAIL default fixed; L14: Magic byte validation on avatar/banner uploads. Accepted risks: M1 (in-memory rate limits, single-instance), M3 (localStorage JWT, SPA architecture), M13 (storage quota, self-hosted), M14 (MIME by extension + magic bytes), M15 (GitHub URL validation). INFO findings I1-I5 accepted as designed.
